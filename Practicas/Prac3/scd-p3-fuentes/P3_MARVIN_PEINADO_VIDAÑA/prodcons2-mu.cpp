...
{
   static int contador = orden*vals_por_prod ;
...
{
   for ( unsigned int i= 0 ; i < vals_por_prod ; i++ )
   {
      // producir valor
...
{
   int id_propio, num_procesos_actual,orden_prod = 0,orden_cons = 0;
   ...
   if ( num_procesos_esperado == num_procesos_actual )
   {
      ...
      if ( id_propio >= 0 && id_propio < num_prod ){
         ...
         orden_cons = id_propio - (num_prod+1);
