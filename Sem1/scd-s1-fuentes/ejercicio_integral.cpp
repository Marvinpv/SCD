...
// 
// NOMBRE:Marvin Peinado Vidaña GRUPO: B3
// -----------------------------------------------------------------------------
...
double funcion_hebra( int i )
{
      double subsuma = 0.0;                        //inicializa la suma
   for(long j = 0 ; j < (m/n) ; j++ ){             //el bucle se repite m/n veces ya que tenemos n hebras
      const double xj = double(i + j*n + 0.5)/m;   //y se coge un punto de la función por cada n 
      subsuma += f( xj );                          //se le añade $f(xj)$ a la suma
   }

   return subsuma/m ;                              //se devuelve el valor de la suma calculada por la hebra
}
...
{
   future<double> subfuturos[ n ];

      for(int i=0 ; i < n ; i++){
         subfuturos[ i ] = async( launch::async , funcion_hebra , i);      //se crean las hebras con la clase future y inicializan la función
      }

      double suma = 0.0;                        //inicializamos la suma 
      for(int i=0 ; i < n ; i++){            
         suma += subfuturos[ i ].get();         //sumamos las distintas partes de área obtenidas por cada hebra
      }

   return suma;                                 //devolvemos el valor final del área
}
// -----------------------------------------------------------------------------
